...
function Login({ setIsLoggedIn, isLoggedIn }) {
    ...
    const handleLogin = (e) => {
        e.preventDefault();
        axios.post("complete-intenship-task-backend.vercel.app/login", { email, password }, { withCredentials: true })
            .then(result => {
                if (result.data === "Success") {
                    axios.get('complete-intenship-task-backend.vercel.app/user', { withCredentials: true })
                        .then(response => {
